...
async def send_welcome_flow(user_id: int, *, max_client: MaxApiClient) -> None:
    await max_client.send_welcome_message(user_id)


async def send_bonus_flow(
    user_id: int,
    phone_number: str,
    *,
    ...
    await max_client.send_message(
        user_id,
        response_text,
        attachments=build_bonus_keyboard(),
    )
...
async def handle_bot_started(
    ...
    *,
    ...
    await send_welcome_flow(user_id, max_client=max_client)
...
async def handle_message_callback(
    ...
    *,
    ...
    if user_id is None or payload not in {CALLBACK_START_PAYLOAD, CALLBACK_CHECK_BONUS_PAYLOAD}:
        return
    ...
    if payload == CALLBACK_START_PAYLOAD:
        logger.info("Received start callback from user_id={}", user_id)
        await send_welcome_flow(user_id, max_client=max_client)
        return

    phone_number = bot_service.get_verified_phone(user_id)
    if not phone_number:
        logger.info("No verified phone for user_id={}, showing welcome flow", user_id)
        await send_welcome_flow(user_id, max_client=max_client)
        return

    logger.info("Rechecking bonus for user_id={}", user_id)
    await send_bonus_flow(
        user_id,
        phone_number,
...
async def handle_message_created(
    ...
    *,
    ...
    if contact_payload is not None:
        ...
        bot_service.remember_verified_phone(user_id, phone_number)
