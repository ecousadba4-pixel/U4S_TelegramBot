...
Под сообщением бота отображаются inline-кнопки:

- при первом входе — **«Поделиться номером телефона»**
- после успешной проверки — **«Проверить бонусы»** (повторный запрос без повторной передачи контакта)

Команда `/start` также доступна через меню команд MAX (иконка «/» рядом с полем ввода).
...
- `telegram_bot_usage_stats` — лог использования (имя таблицы историческое, схема не менялась). При каждой проверке баланса (передача контакта или кнопка «Проверить бонусы») пишется строка: `user_id`, `phone`, `command=contact`.
